    ...
    return (
        ...
        {props.todos.length>0 ? props.todos.map((todo,index)=>{
                return <div key={index} className='todoList'>
                        ...
                        <button onClick={()=>{completeTodo(index)}} className='btn btn-success' style={{color:'blue',textAlign:'center'}}>Completed</button>
                        <button onClick={()=>{deleteTodo(index)}} className='btn btn-warning' style={{color:'blue',textAlign:'center'}}>Delete</button>
                    </div>
